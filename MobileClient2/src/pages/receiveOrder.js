...
class ReceiveOrderPage extends Component {

  static navigationOptions = {
      tabBarLabel: 'Receive Order',
      tabBarIcon: ({tinColor})=>(
        <Image
          source={require('../images/receive_order.png')} style={styles.icon}
        />
    ),
  }
  ...
  componentWillUpdate(nextProps, nextState){
    ...
    if (type == WS.GET_AVAILABLE_ORDERS_RESPONSE){
      ...
      if (result) {
       ...
       nextProps.onGetAvailableOrdersCompleted();
    }
  }
  ...
  getItems(){
    ...
    //  this.props.onGetAvailableOrders(id,position);
    // });
    this.props.onGetAvailableOrders(id,null);
  }
...
const mapActionToProps = (dispatch) => {
  return {
    onGetAvailableOrders: (token, position) => {dispatch(orderActions.getAvailableOrders(token, position));},
    onGetAvailableOrdersCompleted: () => {dispatch({type:WS.GET_AVAILABLE_ORDERS_RESPONSE_COMPLETED});},
  }
}
export default connect(mapStateToProps, mapActionToProps)(ReceiveOrderPage);
