...
class ShareOrderPage extends Component {

  static navigationOptions = {
      tabBarLabel: 'Share Order',
      tabBarIcon: ({tinColor})=>(
        <Image
          source={require('../images/share.png')} style={styles.icon}
        />
    ),
  }
  ...
  getItems(){
      ...
      this.props.onGetShareOrders(id);
  }
  ...
  componentWillUpdate(nextProps, nextState){
    ...
    if (type == WS.GET_SHARE_ORDERS_RESPONSE){
      const {result, orders} = response;
      if (result) {
           //this.showSnackBar("Get Orders Successfully");
           this.setState({orders: orders});

          //  if (this.state.openedOrderID != null){
          //    const {openedOrderID} = nextState;
          //
          //    orders.map((val, key)=>{
          //      if (val.id == openedOrderID){
          //        nextProps.onUpdateOpenedOrder(val);
          //      }
          //    });
           //
          //  }
           ...
           this.showSnackBar("Get Share Orders Fail");
         }
       nextProps.onGetShareOrdersCompleted();
    }
  }
...
const mapActionToProps = (dispatch) => {
  return {
    ...
    onGetShareOrders: (token) => {dispatch(orderActions.getShareOrders(token));},
    onGetShareOrdersCompleted: () => {dispatch({type:WS.GET_SHARE_ORDERS_RESPONSE_COMPLETED});},
    //onCancelOrderSuccess: () => {dispatch(orderActions.cancelOrderSuccess());},
  }
}
export default connect(mapStateToProps, mapActionToProps)(ShareOrderPage);
